...
const navItems = [
  ...
  { label: "Profil Usaha", href: "/umkm/profil", icon: Store },
  { label: "Produk Hijau", href: "/umkm/produk", icon: Package },
  { label: "Ulasan & Rating", href: "/umkm/ulasan", icon: Star },
  { label: "Riwayat Poin", href: "/umkm/poin", icon: History },
  { label: "Pengumuman", href: "/umkm/pengumuman", icon: Megaphone },
];
  ...
  return (
    <div className="min-h-screen bg-[#020402] text-white flex overflow-x-hidden">
      ...
      <aside className="w-64 bg-black/20 backdrop-blur-xl border-r border-white/10 flex flex-col fixed inset-y-0 left-0 z-40">
        ...
        <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            const isActive =
              pathname === item.href ||
              (item.href === "/umkm/dashboard" && pathname === "/umkm");
            return (
              <Link
                key={item.label}
                href={item.href}
      ...
      <div className="flex-1 ml-64 flex flex-col">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                <p className="text-sm font-bold text-white leading-tight">Kopi Dari Hati</p>
                <p className="text-xs text-neutral-400">Pemilik UMKM</p>
              </div>
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 border-2 border-green-500/30 flex items-center justify-center text-black font-black">
                KH
              </div>
            </div>
          </div>
        ...
        {/* Isi halaman */}
        <main className="flex-1 relative p-8">{children}</main>
      </div>
    </div>
  );
}
